...
synthesizer = Synthesizer(
    "./glow-tts_setting/best_model.pth.tar",
    "./glow-tts_setting/config.json",
    None,
    './hifi-gan_setting/best_model_380103.pth.tar',
    './hifi-gan_setting/config.json',
    None,
    None,
    False,
)
...
def generate_tts(texts):
    temp = ""
    with open(f'mix.wav', 'wb') as f:
        for text in normalize_multiline_text(texts):
            temp = temp + text
            # print("temp",temp)
        wav = synthesizer.tts(temp, None, None)
        f.write(IPython.display.Audio(wav, rate=22050).data)
  

def run_speech():
    # playsound.playsound('./mix.wav')
    # os.remove('mix.wav')
    return 

# generate_tts("너이름이 뭐야?")